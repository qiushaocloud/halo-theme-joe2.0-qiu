					...
					.then((res) => {
						...
						if (resD.length === 0) {
							...
							resD.forEach((itm, idx) => {
								var firstCategorieSlug = itm.categories[0] && itm.categories[0].slug;
								if (firstCategorieSlug) {
									// 不显示排除的分类【主要有文章有一个分类不在排除列表则显示】
									var excludeCategorieSlugsStr = ThemeConfig.exclude_categorie_slugs;
									if (
										excludeCategorieSlugsStr && (
											excludeCategorieSlugsStr === firstCategorieSlug
											|| excludeCategorieSlugsStr.indexOf(firstCategorieSlug+',') !== -1
											|| excludeCategorieSlugsStr.indexOf(','+firstCategorieSlug) !== -1
										)
									) {
										return;
