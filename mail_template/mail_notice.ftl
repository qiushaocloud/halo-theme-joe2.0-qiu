<div class="email-page" style="background:#fff;">
  <#--  <link rel="stylesheet" href="https://githubcdn.qiushaocloud.top/gh/qiushaocloud/halo-theme-joe2.0-qiu@master/source/css/min/joe.emoji.min.css">  -->
  ...
  <div class="email-content" style="width:90%;max-width:660px;margin:20px auto 30px;line-height:1.5;text-align:left;overflow:hidden;border-radius:8px;box-shadow:0 2px 12px 0 rgba(0,0,0,0.1);">
    <div style="overflow:hidden;">
      ...
      <div class="email-text" style="padding:20px 28px 10px;background:#fff;">
        ...
        <div style="margin:12px 0;padding:18px 20px;white-space:pre-line;word-break:break-all;color:#6e6e6e;font-size:13px;background:#f8f8f8;background:repeating-linear-gradient(145deg, #f2f6fc, #f2f6fc 15px, #fff 0, #fff 25px);">${content!}</div>
  ...
  <#--  <script src="https://githubcdn.qiushaocloud.top/gh/qiushaocloud/halo-theme-joe2.0-qiu@master/source/lib/j-marked/marked.min.js"></script>
  <script src="https://githubcdn.qiushaocloud.top/gh/qiushaocloud/halo-theme-joe2.0-qiu@master/source/js/min/joe.utils.min.js"></script>
  <script>
    /* 渲染内容中的 emoji */
    var elm = document.querySelector(".email-text pre");
    // 获取转换后的marked
    var markedHtml = marked(elm.innerHTML).replace(/bili\//g, "bili/hd/ic_emoji_");
    // 处理其中的表情包
    var emoji = Utils.renderedEmojiHtml(markedHtml);
    // 将回车转换为br
    elm.innerHTML = Utils.return2Br(emoji);
  </script>  -->
</div>